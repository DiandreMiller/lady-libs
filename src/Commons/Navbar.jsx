...
const Navbar = () => {
  ...
  const mobileNavItems = [
    ...
    { label: "Wall Art", to: "/inventory/wallart" }, 
    { label: "About Me", to: "/aboutme" },
    { label: "Contact Me", to: "/contactme" }, 
  ];
    ...
    <nav className="w-full fixed top-0 left-0 z-50 backdrop-blur-xl bg-purple-900/30 border-b border-purple-700/40 shadow-lg shadow-purple-900/40">
      ...
      {open && (
        <>
          {/* Backdrop */}
          <div 
            className="sm:hidden fixed top-0 left-0 right-0 bottom-0 z-40 bg-black/50"
            onClick={() => setOpen(false)}
          />
          
          {/* Dropdown panel - EXPLICIT VERSION */}
          <div 
            className="sm:hidden fixed top-16 left-0 right-0 bottom-0 z-50 bg-purple-950/95 backdrop-blur-xl overflow-hidden"
            style={{ 
          ...
          >
            <div className="h-full overflow-y-auto mx-auto max-w-6xl px-4 py-3 space-y-2 pb-24">
              ...
              <button
                ...
                className="mt-3 mb-8 w-full rounded-full bg-gradient-to-r from-yellow-200 to-orange-300 px-4 py-2.5 text-sm font-semibold text-purple-900 shadow-md shadow-purple-900/60 hover:brightness-110 active:scale-95 transition">
                View Cart
              </button>
            </div>
          </div>
